...
import { useExchangeRateScheduler } from './use-exchange-rate-scheduler'
...
export function ExchangeRateSchedulerPage() {
  const { executions, loading, error, isRunning, triggerRun, runError, deleteExecution, deletingId, deleteError } =
    useExchangeRateScheduler()

  const displayError = error ?? runError ?? deleteError

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">환율수집 스케줄러</h1>
          <p className="mt-1 text-sm text-surface-500">
            매일 19:00 KST 자동 실행 · USD/KRW 환율 수집
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-surface-500">
          ...
          <p className="mt-1 text-sm">수동 실행 버튼을 눌러 환율 수집을 시작하세요.</p>
        </div>
      ) : (
