...
- **[v1.1 — Fetch timeout & abort signal propagation]**: Pass `AbortSignal` through `withRetry` → adapter `fetchFn` so that in-flight HTTP requests are cancelled on abort. Add per-request timeout (default 30s) to prevent indefinite hangs from unresponsive upstream APIs.
...
- ~~Hard kill of the currently-processing individual product (mid-product interruption is not safe)~~ → Moved to In Scope (v1.1): abort signal now propagated to fetch calls for safe mid-request cancellation
...
| 9 | **[v1.1]** Fetch timeout prevents hang | Given a product's upstream API (Naver/Yahoo) does not respond within 30 seconds, when the fetch times out, then the product is marked as failed and processing continues to the next product. The scheduler does not hang indefinitely. |
| 10 | **[v1.1]** Abort cancels in-flight fetch | Given an execution is running and a product fetch is in-flight, when `POST /stop` is called, then the in-flight HTTP request is cancelled via `AbortSignal`, the product is marked as failed (or skipped), and the execution finalizes with `status = 'aborted'` within seconds. |
| 11 | **[v1.1]** Abort cancels retry attempts | Given `withRetry` is retrying a failed fetch, when the abort signal fires, then no further retry attempts are made and the function throws immediately. |
...
**Problem**: The current abort mechanism only checks `signal.aborted` between products/batches. If an upstream API hangs (Naver or Yahoo), the scheduler blocks indefinitely on that single fetch — even after abort is triggered.

**Solution**: Thread `AbortSignal` through the entire call chain:

```
PriceCollectorService.run()
  → AbortController created
    → processProducts(signal)
      → processBatch(signal)
        → processOneProduct(signal)
          → withRetry(fn, { signal })          // NEW: bail on abort
            → fetchFn(product, range, signal)   // NEW: pass signal
              → fetch(url, { signal })          // Naver: native abort
              → client.historical({}, signal)   // Yahoo: abort support
```
...
**Naver adapter changes** — pass `signal` to `fetch()` + add 30s timeout:

```typescript
async fetchPrices(code, productId, startDate, endDate, signal?: AbortSignal) {
...
**Yahoo adapter changes** — wrap with `AbortSignal.timeout`:
...
async fetchPrices(code, productId, startDate, endDate, signal?: AbortSignal) {
  ...
  const combinedSignal = signal
    ? AbortSignal.any([signal, timeoutSignal])
    : timeoutSignal
  const rows = await this.client.historical(code, { ... }, { signal: combinedSignal })
  // ...
}
```

**`AdapterFetchFn` type updated**:
...
|----------|---------|-------------|
| `PRICE_FETCH_TIMEOUT_MS` | `30000` | Per-request timeout for upstream API calls |
...
| 6 | **[v1.1]** Tests (RED): `withRetry` bails immediately when signal is aborted; no further retries after abort. Update `withRetry` signature to accept `{ signal }` options (GREEN). | Low |
| 7 | **[v1.1]** Tests (RED): `NaverFinanceAdapter.fetchPrices` respects abort signal and 30s timeout; `YahooFinanceAdapter.fetchPrices` respects abort signal and 30s timeout. Update both adapters (GREEN). | Medium |
| 8 | **[v1.1]** Thread `AbortSignal` through `PriceCollectorService`: `processOneProduct` → `withRetry` → adapter `fetchFn`. Update `AdapterFetchFn` type. Update existing tests. | Medium |
...
- [ ] **[v1.1]** Abort signal is passed to `fetch()` in `NaverFinanceAdapter` — in-flight HTTP request is cancelled when abort fires
- [ ] **[v1.1]** Abort signal is passed to `yahoo-finance2` client — in-flight request is cancelled when abort fires
- [ ] **[v1.1]** `withRetry` stops retrying immediately when abort signal fires (no further attempts)
- [ ] **[v1.1]** Per-request timeout of 30s (configurable via `PRICE_FETCH_TIMEOUT_MS`) prevents indefinite hang on unresponsive APIs
- [ ] **[v1.1]** After `POST /stop`, the execution finalizes within ~30 seconds (worst case: one in-flight request timeout)
...
|------|--------|------------|
| ~~Abort signal checked between batches, not mid-product~~ | ~~Low~~ | **Resolved in v1.1**: abort signal now propagated to fetch calls; `AbortSignal.timeout(30s)` prevents indefinite hangs. |
| `AbortSignal.any()` requires Node.js 20+ | Low | Project already targets Node 22 (see Dockerfile). No compatibility issue. |
| `yahoo-finance2` may not fully support `AbortSignal` | Medium | Wrap call in `Promise.race` with timeout as fallback if signal is ignored. Test with actual abort. |
...
| 2026-03-13 | 1.1 | - | Added fetch timeout (30s) and abort signal propagation to fix scheduler hanging on unresponsive upstream APIs. Waves 6-8 added. Moved "mid-product interruption" from Out of Scope to In Scope. Updated implementation-status to completed (v1.0 abort mechanism already implemented). |
